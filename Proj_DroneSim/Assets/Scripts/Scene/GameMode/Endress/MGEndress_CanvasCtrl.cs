...
/// 　(シーンのから呼び出す)
/// </summary>
public class MGEndress_CanvasCtrl : ShCanvasBaseCtrl, IShVisible
{
    //================================================
    // [///]  各シーンごとに保持する変数
    //================================================
    ...
    public override void ProcInitialize()
    {
        ...
        // 自身から子供の表示制御の付与取得
        {
            ...
            m_UIList.Clear();
            {
                ...
                m_UIList.Add(canvasT.Find("Grp_Hader/Txt_ZDist").gameObject.AddComponent<ShUITMPTextCtrl>());
            }
    ...
    // タイマーの取得
    public void SetZDistText(string text)
    {
        ((ShUITMPTextCtrl)m_UIList.m_List[(int)UINo.Text_ZDist]).SetText("飛距離:" + text);
    }
    // リザルト
    public void SetResult(int score, bool isFailed=false)
    {
        ...
        if (isFailed)
        {
            ((ShUITMPTextCtrl)m_UIList.m_List[(int)UINo.Result_Mission]).SetText("スコア");
            ((ShUITMPTextCtrl)m_UIList.m_List[(int)UINo.Result_Mission]).m_Text.color = Color.yellow;

            {
                TMPro.TMP_Text text = m_ThisCanvas.transform.Find("Grp_Result/Txt_ZZZ").GetComponent<TMPro.TMP_Text>();
                text.text = (MGEndress_Mediator.ObjMediator.DroneCtrl.transform.position.z * 0.1f).ToString("f1") + "m";
            }
    ...
    {
        MainGame.MGEndress_Mediator.StateCtrl.SetMainGameReset();
    }
    public void OnClick_Settings()
    {
        // 開けるか
        if(!MGEndress_Mediator.StateCtrl.IsUseMenu()) return;

        // このタイミングではゲームを停止する
        MGEndress_Mediator.StateCtrl.CallGameWait();
    }
    public void OnClick_Return_Settings()
    {
        // このタイミングではゲームを再開する
        MGEndress_Mediator.StateCtrl.CallGameReStart();
    }
    // 戻る
    public void OnClick_ReturnScene()
    {
        // ドローン操作ログの設定
        MGEndress_Mediator.ObjMediator.SaveDroneInputLog();

        // @取り急ぎ
        ShSceneUtils.GetAppCtrlObj(AppCommon.CurrentSceneName).GetComponent<Game_EndressBaseCtrl>().ProcChangeScene();
    }
    // リプレイ
    public void OnClick_Replay()
    {
        ...
        // @取り急ぎ
        ShSceneUtils.GetAppCtrlObj(AppCommon.CurrentSceneName).GetComponent<Game_EndressBaseCtrl>().ProcChangeScene_Replay();
    }
}

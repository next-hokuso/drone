...
namespace MainGame
{
    ...
    /// 制御スクリプト：MainGame
    /// </summary>
    public class Game_EndressBaseCtrl : ShSceneBaseCtrl
    {
        //================================================
        // [///] 定義
        //================================================
        // シーン名
        public const string SceneName = "Game_Endress";
        // メインカメラ名
        ...
        // ゲーム状態管理
        private MGEndress_GameStateCtrl m_StateCtrl = null;
        // オブジェクト管理
        private MGEndress_ObjMediator m_ObjMediator = null;

        // メインゲームキャンバス
        private MGEndress_CanvasCtrl m_CanvasCtrl = null;
        ...
        public override IEnumerator ProcResourcesLoad()
        {
            ...
            m_CanvasCtrl = gameObject.AddComponent<MGEndress_CanvasCtrl>();
            ...
            // MGEndress_Mediator セットアップ
            {
                // state管理付与
                m_StateCtrl = gameObject.AddComponent<MGEndress_GameStateCtrl>();
                MGEndress_Mediator.SetGameStateCtrl(m_StateCtrl);
                // 配置ルート
                MGEndress_Mediator.SetRootT(m_SetRoot);
                // オブジェクト管理
                m_ObjMediator = new MGEndress_ObjMediator();
                m_ObjMediator.SetInit();
                MGEndress_Mediator.SetObjMediator(m_ObjMediator);
                // キャンバス
                MGEndress_Mediator.SetCanvas(m_CanvasCtrl);
                // カメラ
                GameObject goCamera = ShSceneUtils.GetSceneRootObj(SceneName, "CamRoot");
                MGEndress_Mediator.SetCamera(goCamera.transform.Find("ObjCamera").GetComponent<Camera>());
                GameObject uiCamera = ShSceneUtils.GetSceneRootObj(SceneName, "Main Camera");
                MGEndress_Mediator.SetUICamera(uiCamera.GetComponent<Camera>());
        ...
        private IEnumerator AddtiveScene()
        {
            ...
            // 
            //     // MGEndress_Mediatorの設定
            //     MGEndress_Mediator.SetTitleCanvas(canvasCtrl);
            // }

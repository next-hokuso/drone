...
/// 　(シーンのから呼び出す)
/// </summary>
public class MGTreasure_CanvasCtrl : ShCanvasBaseCtrl, IShVisible
{
    //================================================
    // [///]  各シーンごとに保持する変数
    //================================================
    ...
    {
        MainGame.MGTreasure_Mediator.StateCtrl.SetMainGameReset();
    }
    public void OnClick_Settings()
    {
        // 開けるか
        if(!MGTreasure_Mediator.StateCtrl.IsUseMenu()) return;

        // このタイミングではゲームを停止する
        MGTreasure_Mediator.StateCtrl.CallGameWait();
    }
    public void OnClick_Return_Settings()
    {
        // このタイミングではゲームを再開する
        MGTreasure_Mediator.StateCtrl.CallGameReStart();
    }
    // 戻る
    public void OnClick_ReturnScene()
    {
        // ドローン操作ログの設定
        MGTreasure_Mediator.ObjMediator.SaveDroneInputLog();

        // @取り急ぎ
        ShSceneUtils.GetAppCtrlObj("Game_Treasure").GetComponent<Game_TreasureBaseCtrl>().ProcChangeScene();
    }
    // リプレイ
    public void OnClick_Replay()
    {
        ...
        // @取り急ぎ
        ShSceneUtils.GetAppCtrlObj("Game_Treasure").GetComponent<Game_TreasureBaseCtrl>().ProcChangeScene_Replay();
    }
}

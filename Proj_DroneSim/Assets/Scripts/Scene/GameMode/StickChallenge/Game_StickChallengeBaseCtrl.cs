...
namespace MainGame
{
    ...
    /// 制御スクリプト：MainGame
    /// </summary>
    public class Game_StickChallengeBaseCtrl : ShSceneBaseCtrl
    {
        //================================================
        // [///] 定義
        //================================================
        // シーン名
        public const string SceneName = "Game_StickChallenge";
        // メインカメラ名
        ...
        // ゲーム状態管理
        private MGStickChallenge_GameStateCtrl m_StateCtrl = null;
        // オブジェクト管理
        private MGStickChallenge_ObjMediator m_ObjMediator = null;

        // メインゲームキャンバス
        private MGStickChallenge_CanvasCtrl m_CanvasCtrl = null;
        ...
        public override IEnumerator ProcResourcesLoad()
        {
            ...
            m_CanvasCtrl = gameObject.AddComponent<MGStickChallenge_CanvasCtrl>();
            ...
            // MGStickChallenge_Mediator セットアップ
            {
                // state管理付与
                m_StateCtrl = gameObject.AddComponent<MGStickChallenge_GameStateCtrl>();
                MGStickChallenge_Mediator.SetGameStateCtrl(m_StateCtrl);
                // 配置ルート
                MGStickChallenge_Mediator.SetRootT(m_SetRoot);
                // オブジェクト管理
                m_ObjMediator = new MGStickChallenge_ObjMediator();
                m_ObjMediator.SetInit();
                MGStickChallenge_Mediator.SetObjMediator(m_ObjMediator);
                // キャンバス
                MGStickChallenge_Mediator.SetCanvas(m_CanvasCtrl);
                // カメラ
                GameObject goCamera = ShSceneUtils.GetSceneRootObj(SceneName, "CamRoot");
                MGStickChallenge_Mediator.SetCamera(goCamera.transform.Find("ObjCamera").GetComponent<Camera>());
                GameObject uiCamera = ShSceneUtils.GetSceneRootObj(SceneName, "Main Camera");
                MGStickChallenge_Mediator.SetUICamera(uiCamera.GetComponent<Camera>());
        ...
        private IEnumerator AddtiveScene()
        {
            ...
            // 
            //     // MGStickChallenge_Mediatorの設定
            //     MGStickChallenge_Mediator.SetTitleCanvas(canvasCtrl);
            // }
